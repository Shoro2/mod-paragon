...
- **Self-healing reconcile (2026-07-19).** The same DB reconcile also runs on every map change and resurrect (cheap PK SELECT, no-op diff when nothing drifted), and every reconcile first purges the legacy stacking auras. This makes the module immune to snapshot-vs-DB drift — e.g. a stale pre-redesign Lua deployment that stacks legacy auras and never casts 100028 (root cause of the 2026-07 "stats/Life Leech vanish after wipes/teleports" report). **Deploy contract:** `lua_scripts/Paragon_System/` must always match `Paragon_System_LUA/`.
- **All paragon auras are hidden.** The level marker 100000 carries `PASSIVE|DO_NOT_DISPLAY` (patched into server + client Spell.dbc via `ForgottenLand2.0/scripts/35_hide_paragon_marker_dbc.py`); passive auras are never sent to the client, so no paragon buff icon appears. 100028/100029 carry `DO_NOT_DISPLAY` + corrected death-persist bits in `spell_dbc` (`fix_paragon_spell_attributes.sql`).
