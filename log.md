...
- 2026-07-11 — fix(Loader): rename AddMyNPCScripts to AddParagonNPCScripts — generic `AddMy*` loader symbols collide across statically linked module libs (fl-underground-dungeon's AddMyPlayerScripts was shadowed by mod-dungeonmaster and never linked); unique name removes mod-paragon from that hazard class.
- 2026-05-27 — fix(Paragon/Lua): validate AllocatePoint/DeallocatePoint args via Dep_Validation — `statId` (1..#STATS) and `amount` (positive int ≤ 666) are checked through `share-public/AIO_Server/Dep_Validation/validation.lua` (with a permissive shim fallback + warning if the lib is not deployed). Ported the input-validation idea from the abandoned PR #42 onto the new C++-single-source-of-truth `Paragon_Server.lua`.
