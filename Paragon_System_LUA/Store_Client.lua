...
--function StoreHandler.UpdateParagon(player, paragonData)
	--SHOP_UI.ParagonDataUpdate(paragonData)
--end
...
function SHOP_UI.MainFrame_Create()
	...
	shopFrame.Title:SetText("|cffedd100Character Upgrades|r")
...
function SHOP_UI.OnSellConfirm(data)
...
function SHOP_UI.ServiceBoxes_Create(parent)
	...
	for i = 1, 8 do
		...
		service.sellButton:SetScript(
			"OnClick",
			function(self)
				AIO.Handle("STORE_SERVER", "Sell", self:GetParent().ServiceId * 10)
				PlaySound("STORE_CONFIRM", "Master")
			end
		)
...
function SHOP_UI.ServiceBoxes_Update()
	...
	for i, serviceData in ipairs(services) do
		if i >= startIndex and i <= endIndex then
			...
			service.ParagonDisplay:SetFormattedText("|cffffffff%s|r", "0/255")
...
function SHOP_UI.ParagonDataUpdate(paragonData)
	for i = 21, 25, 1 do
		local service = GetServiceData(i)
		for s, service in pairs(Services) do
			service.ParagonDisplay:SetFormattedText("|cffffffff%s|r", paragonData[i-21].."/255")
		end
	end
end
...
local function ModifyGameMenuFrame()
	...
	storeButton.Text:SetText("|cffdbe005Paragon");
