...
function GetParagonData(characterID)
	ParagonData.Cache = {};
	local Query = CharDBQuery("SELECT * FROM character_paragon_points WHERE characterID ="..characterID)
	if (Query) then
		repeat
			ParagonData.Cache = {
				Query:GetUInt32(1), -- strength
				Query:GetString(2), -- intellect
				Query:GetString(2), -- agility
				Query:GetString(2), -- spirit
				Query:GetString(2), -- stamina
			}
		until not Query:NextRow()
	end
